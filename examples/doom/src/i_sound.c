...
void I_InitMusic(void)
{
    ...
    music_okay = 0;
    mphnd = 0;//EMAC: redo//dfs_open("MIDI_Instruments");
    if (-1 == mphnd)
    {
        //EMAC:printf("I_InitMusic: Could not open MIDI_Instruments file (%s)\n", strerror(errno));
        printf("             MUS playback is disabled.\n");
        return;
    }

    rc = 0;//EMAC: redo//dfs_read(midi_pointers, sizeof(uint32_t), NUM_MIDI_INSTRUMENTS, mphnd);
    //EMAC: redo//dfs_close(mphnd);
    if ((sizeof(uint32_t)*NUM_MIDI_INSTRUMENTS) != rc)
    {
        //EMAC:printf("I_InitMusic: Could not read MUS instrument headers (%s)\n", strerror(errno));
        printf("             MUS playback is disabled.\n");
        return;
    }
    ...
    for (i = 0; i < NUM_MIDI_INSTRUMENTS; i++)
    {
        ...
        if (instrument_used(i))
        {
...
#endif
            {
                ...
                hnd = 0;//EMAC: redo//dfs_open("MIDI_Instruments");
                if (hnd < 0)
                {
#ifdef RANGECHECK
                    //EMAC:I_Error("Mus_Register: Could not open MIDI_Instruments file (%s)\n", strerror(errno));
#else
                ...
                //EMAC: redo//dfs_seek(hnd, instrdata_ptr, SEEK_SET);

                if (sizeof(struct midiHdr) != 0)//EMAC: redo//dfs_read(mhdr, sizeof(uint8_t), sizeof(struct midiHdr), hnd))
                {
#ifdef RANGECHECK
                    I_Error("Mus_Register: Could not read header for instrument %d from MIDI_Instruments file.\n", i);
#else
                    //EMAC: redo//dfs_close(hnd);
                    free(mhdr);
#endif
                }
                ...
                if (sample)
                {
                    //EMAC: redo//dfs_seek(hnd, instrdata_ptr + offsetof(struct midiHdr, sample), SEEK_SET);
                    //EMAC: redo//dfs_read(sample, sizeof(int8_t), length, hnd);
