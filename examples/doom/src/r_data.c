...
void R_GenerateLookup (int texnum)
{
	...
	if (!patchcount[x])
	{
        I_Error("R_GenerateLookup: column without a patch (%s)\x0a\x0d", texture->name);
	    return;
	}
#endif
	if (patchcount[x] > 1)
...
int	R_CheckTextureNumForName (char *name)
{
    ...
    for (i=0 ; i<numtextures ; i++)
    {
        //EMAC:if (!strncasecmp (textures[i]->name, name, 8 /*sizeof(texture_t->name)*/  ) )
        if (!stricmp2 (textures[i]->name, name, 8 /*sizeof(texture_t->name)*/  ) )
        {
	    return i;
        }
    }
