...
{ 
    ...
    //EMAC:printf("G_Ticker1: gametic=%08x\x0a\x0d", gametic);

    // do things to change the game state
    while (gameaction != ga_nothing) 
    { 
        //EMAC:printf("\tgameaction=%08x\x0a\x0d", gameaction);
    switch (gameaction) 
    { 
      case ga_loadlevel: 
        G_DoLoadLevel (); 
    ...
    //EMAC:printf("\tgamestate=%08x\x0a\x0d", gamestate);
    // do main actions
    switch (gamestate) 
    { 
      case GS_LEVEL: 
...
{ 
    ...
    if (i && i<myargc-1)
        maxsize = 0x20000;//EMAC:atoi(myargv[i+1])*1024;
