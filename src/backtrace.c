...
extern uint32_t exception_handler_end[];
...
/** @brief Start of the code section (see wtv.ld) */
extern uint32_t __code_start[];
/** @brief End of the code section (see wtv.ld) */
...
static symtable_header_t symt_open(void) {
    ...
    if (symt_header.head[0] != 'S' || symt_header.head[1] != 'Y' || symt_header.head[2] != 'M' || symt_header.head[3] != 'T') {
        debugf("backtrace: invalid symbol table found at 0x%08x\n", SYMT_ROM);
        SYMT_ROM = 0;
        return (symtable_header_t){0};
    }
...
{
    /*EMAC:assert(idx >= 0 && idx < symt->addrtab_size);
    return io_read(SYMT_ROM + symt->addrtab_off + idx * 4);*/
    return (addrtable_entry_t){0};
}
...
{
    /*EMAC:data_cache_hit_writeback_invalidate(entry, sizeof(symtable_entry_t));
...
{
    if (addr >= (uint32_t)exception_handler && addr < (uint32_t)exception_handler_end) {
...
char* __symbolize(void *vaddr, char *buf, int size)
{
    ...
    if (symt.head[0]) {
        ...
        char lbuf[12];
        snprintf(lbuf, sizeof(lbuf), "+0x%x", addr - ADDRENTRY_ADDR(a));
...
{
    *func = (bt_func_t){
        .type = (ptr >= exception_handler && ptr < exception_handler_end) ? BT_EXCEPTION : BT_FUNCTION,
        ...
        switch (func.type) {
            ...
            case BT_EXCEPTION: {
                ...
                if (symt.head[0]) {
                    ...
                    #if BACKTRACE_DEBUG
                    debugf("Found interrupted function start address: %08x\n", func_start);
                    #endif
                }
            }   break;
            case BT_LEAF:
    ...
    void (*cb)(void *, backtrace_frame_t *), void *cb_arg)
{
    ...
    for (int i=0; i<size; i++) {
        ...
        /*EMAC:if (!is_valid_address(needle)) {
        ...
        if (!has_symt) {*/
            ...
            bool exc = (needle >= (uint32_t)exception_handler && needle < (uint32_t)exception_handler_end);
            cb(cb_arg, &(backtrace_frame_t){
                .addr = needle,
...
char** backtrace_symbols(void **buffer, int size)
{
    ...
    void cb(void *arg, backtrace_frame_t *frame) {
        int n = snprintf(out, MAX_SYM_LEN,
            "%s+0x%x (%s:%d) [0x%08x]", frame->func, frame->func_offset, frame->source_file, frame->source_line, frame->addr);
        if (frame->is_inline)
            out[-1] = '\n';
        else
            syms[level++] = out;
...
void backtrace_frame_print(backtrace_frame_t *frame, int *out)
{
    printf("%s+0x%x (%s:%d) [0x%08x]%s", 
        frame->func, frame->func_offset, 
        frame->source_file, frame->source_line,
        frame->addr, frame->is_inline ? " (inline)" : "");
}

void backtrace_frame_print_compact(backtrace_frame_t *frame, int *out, int width)
{
    ...
    if (frame->func != UNKNOWN_SYMBOL) printf("%s ", frame->func);
    if (source_file != UNKNOWN_SYMBOL) printf("(%s%s:%d)", ellipsed ? "..." : "", source_file, frame->source_line);
    if (frame->func == UNKNOWN_SYMBOL || source_file == UNKNOWN_SYMBOL)
        printf("[0x%08x]", frame->addr);
    printf("\n");
}
